...
export async function syncUnits(config: SyncConfig): Promise<SyncResult> {
  ...
  // Track connection errors globally
  ...
  const categorizeError = (code: string, error: any): void => {
    ...
    if (errorMsg.includes('ENOTFOUND') || errorMsg.includes('ECONNREFUSED') ||
      ...
      errorMsg.includes('ERR_INTERNET_DISCONNECTED') || errorMsg.includes('DNS')) {
      if (!connectionErrorShown) {
        console.log('\n❌ CONNECTION ERROR: Unable to reach training.gov.au');
  ...
  for (let i = 0; i < allUnitsToProcess.length; i += CONCURRENT_BATCH) {
    ...
    await Promise.all(batch.map(async (code) => {
      ...
      try {
        ...
        const errorMsg = error.message || String(error);
        // Only show individual error if not a connection error (we already showed global message)
        if (!connectionErrorShown) {
  ...
  const crawler = new Crawler(fetcher, exporter, {
    ...
    onItem: (item) => {
      scraped++;
      const percent = Math.round((scraped / validUnits.length) * 100);
      const elemCount = item.elements?.length || 0;
      const pcCount = item.elements?.reduce((s, e) => s + e.performanceCriteria.length, 0) || 0;
      console.log(`[${scraped}/${validUnits.length}] ${percent}% ✅ ${item.code} - ${item.title}`);
...
// Run main if this file is executed directly or via tsx
// Simplified check to ensure it runs
