...
export async function extractQuestionsFromDocx(
    ...
    try {
        ...
        if (messages.length > 0) {
            logger.warn('   ⚠️ Mammoth messages:', messages);
        }

        logger.info(`   📄 DOCX converted to HTML (${html.length} chars). Parsing structure...`);
        ...
        $('p, li, tr').each((i, elem) => {
            ...
            if ((isHeader || (isBold && text.length < 100) || (sectionMatch && text.length < 50))) {
                if (text.match(/^(PART|SECTION|MODULE|UNIT|KNOWLEDGE|PRACTICAL|ASSESSMENT)\s/i) || sectionMatch) {
                    ...
                    logger.info(`   📂 Section detected: ${currentSection}`);
                    return;
                }
            }
            ...
            if (isInstruction) {
                ...
                if (!isDuplicate) {
                    ...
                    logger.info(`   📋 Captured instruction: ${text.substring(0, 50)}...`);
                } else {
                    // logger.debug(`   ⏭️  Skipping duplicate instruction: ${text.substring(0, 50)}...`);
                }
                return;
            }
            ...
            textParts.forEach(partText => {
                ...
                if (tableQuestion && textParts.length === 1) {
                    ...
                    if (!isClearlyContinuation) {
                        ...
                        pQuestionText = partText;
                        pIsNewQuestion = true;
                        // logger.debug(`   🔍 Treating text as new question: ${partText.substring(0, 40)}...`);
                    }
                }
                ...
                if (pIsNewQuestion && pQuestionId && pQuestionText) {
                    ...
                    for (let i = 0; i < questions.length; i++) {
                        ...
                        if (normalizedText === existingNorm) {
                            isDuplicate = true;
                            // logger.debug(`   ⏭️  Skipping exact duplicate: ${pQuestionText.substring(0, 40)}...`);
                            break;
                        }
                    ...
                    if (replaceIndex !== -1) {
                        ...
                        logger.info(`   🔄 Replaced Q${replaceIndex + 1} with longer version`);
                    } else {
                        questions.push(newQuestion);
                        logger.info(`   ✓ Extracted Q${questions.length}: ${uniqueId}`);
                    }
                } else {
                    // Append to previous
                    if (questions.length > 0) {
                        ...
                        if (partText.length > 3 && (normLastQ.includes(normText) || normLastQ.endsWith(normText))) {
                            // logger.debug(`   Skipping duplicate text append: ${partText.substring(0, 20)}...`);
                            return;
                        }
        ...
        logger.info(`   🔍 Detected Unit Codes: ${detectedUnitCodes.join(', ')}`);

        return { questions, detectedUnitCodes, instructions };

    } catch (error) {
        logger.error('Error extracting questions from DOCX:', error);
        throw error;
    }
}
